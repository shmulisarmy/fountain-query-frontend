...
export async function run_tests() {
  ...
  const client_data_reflection_1: { [key: string]: Person; } = live_db(`${ws_url(backend_base_url)}/stream-data`);
  await fetch(`${http_url(backend_base_url)}/add-person?name=donkey`);
  const client_data_reflection_2: { [key: string]: Person; } = live_db(`${ws_url(backend_base_url)}/stream-data`);
  await sleep(200);
  ...
  const client_data_reflection_3: { [key: string]: Person; } = live_db(`${ws_url(backend_base_url)}/stream-data`);
  await sleep(200);
